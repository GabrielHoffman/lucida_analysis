---
title: "Analysis of 1K1K single cell dataset with lucida"
subtitle: '[Yazar, et al. (2022)](https://doi.org/10.1126/science.abf3041)'
author: "Developed by [Gabriel Hoffman](http://gabrielhoffman.github.io/)"
date: "Run on `r format(Sys.time())`"
documentclass: article
vignette: >
  %\VignetteIndexEntry{Analysis of 1K1K single cell dataset with lucida}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
  %\usepackage[utf8]{inputenc}
output:
  BiocStyle::html_document:
    toc: true
    toc_float: true
---

<!---

cd /hpc/users/hoffmg01/work/lucida_analysis
module load pandoc imagemagick
git pull

# scp minerva:/hpc/users/hoffmg01/work/lucida_analysis/1k1k_analysis.html .

# rm -rf 1k1k_analysis_cache/

rmarkdown::render("1k1k_analysis.Rmd")

system("cp -f 1k1k_analysis.html ~/www/")


https://hoffmg01.dmz.hpc.mssm.edu/1k1k_analysis.html


ml purge
ml anaconda3/latest
conda activate jul2026

# format H5AD
SRC=/hpc/users/hoffmg01/work/GenomicDataStream_analysis/recode_h5ad.py 
FILE=/sc/arion/projects/CommonMind/hoffman/scRNAseq_data/Yazar_Science_2022/08984b3c-3189-4732-be22-62f1fe8f15a4.h5ad

$SRC --input $FILE --sortBy cell_type,donor_id --format CSC --compression lzf --out $(dirname $FILE)/1k1k_sort_CSC_lzf.h5ad

$SRC --input $FILE --sortBy cell_type,donor_id --format CSC --compression zstd --out $(dirname $FILE)/1k1k_sort_CSC_zstd.h5ad

--->


```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = TRUE,
  warning = FALSE,
  message = FALSE,
  error = FALSE,
  tidy = FALSE,
  dev = c("png", "pdf"),
  cache = TRUE,
  lazy.cache = FALSE
)
```

```{r packages, cache=FALSE}
library(lucida)
library(GenomicDataStream)
library(SingleCellExperiment)
library(aplot)
library(tidyverse)
library(ggplot2)
library(nebula)

thm = theme_classic(15) +
    theme(aspect.ratio = 1, 
      plot.title = element_text(hjust = 0.5)) 
```

# Lucida analysis
```{r load, cache=FALSE}
file = "/sc/arion/projects/CommonMind/hoffman/scRNAseq_data/Yazar_Science_2022/1k1k_sort_CSC_lzf.h5ad"
# file = "~/Downloads/1k1k_sort_CSC_lzf.h5ad"
sce = readH5AD(file)

# filter to include only cell types with 1k observations
tab = table(sce$cell_type)
sce = sce[,sce$cell_type %in% names(tab[tab>1000])]
sce = sce[,sce$libSize > 1000]
sce$cell_type = droplevels(sce$cell_type)
```

```{r summary, fig.height = 15, fig.width=15, cache=FALSE}
fig1 = colData(sce) %>%
  as.data.frame %>%
  as_tibble %>%
  ggplot(aes(libSize, cell_type, fill=cell_type)) +
    geom_violin(linewidth = 0) +
    geom_boxplot(width=.1, linewidth = 0.5) +
    scale_x_log10() +
    theme_classic() +
    theme(aspect.ratio = 2, 
      plot.title = element_text(hjust = 0.5), 
      legend.position="none") +
    xlab("Number of reads per cell (log10)")

fig2 = colData(sce) %>%
  as.data.frame %>%
  as_tibble %>%
  group_by(cell_type, donor_id) %>%
  summarize(nCells = n()) %>%
  ggplot(aes(nCells, cell_type, fill=cell_type)) +
    geom_violin(linewidth = 0) +
    geom_boxplot(width=.1, linewidth = 0.5) +
    scale_x_log10() +
    theme_classic() +
    theme(aspect.ratio = 2, 
      plot.title = element_text(hjust = 0.5), 
      legend.position="none",
      axis.title.y = element_blank(),
      axis.text.y = element_blank()) +
    xlab("Number of cells per donor (log10)")

fig1 | fig2
```


```{r lucida}
fit = lucida(sce, ~ age + sex + (1|donor_id), 
  cluster_id = "cell_type", nReaders=6)
```

## Plots
```{r plotDispEsts, fig.height = 15, fig.width = 15}
plotDispEsts(fit, ncol=5)
```

```{r plotMA, fig.height = 15, fig.width = 15}
plotMA(fit, coef='age', ncol=5)
plotMA(fit, coef='sexmale', ncol=5)
```

```{r plotVolcano, fig.height = 15, fig.width = 15}
plotVolcano(fit, coef='age', ncol=5)
plotVolcano(fit, coef='sexmale', ncol=5)
```


```{r vp, fig.height = 15, fig.width =15}
vp <- varpart(fit)
 
plotVarPart(sortCols(vp), ncol=5)
```

```{r vp2, fig.height = 15, fig.width =15}
plotTrendVP( fit, vp, "CountNoise")
```

# Results
```{r results}
results(fit, "age") %>%
  group_by(cluster_id) %>%
  summarize( n = n()) %>%
  ggplot(aes(n, cluster_id, fill=cluster_id)) +
    geom_bar(stat="identity") +
    thm +
    theme(aspect.ratio = 2, legend.position="none") +
    scale_x_continuous(limits=c(0,NA), expand=c(0,0)) +
    xlab("# genes")

figAge = results(fit, "age") %>%
  group_by(cluster_id) %>%
  summarize( nSignif = sum(FDR < 0.05)) %>%
  ggplot(aes(nSignif, cluster_id, fill=cluster_id)) +
    geom_bar(stat="identity") +
    thm +
    theme(aspect.ratio = 2, legend.position="none") +
    scale_x_continuous(limits=c(0,NA), expand=c(0,0)) +
    xlab("# significant genes") +
    ggtitle("Age")
figAge

results(fit, "sexmale") %>%
  group_by(cluster_id) %>%
  summarize( nSignif = sum(FDR < 0.05)) %>%
  ggplot(aes(nSignif, cluster_id, fill=cluster_id)) +
    geom_bar(stat="identity") +
    thm +
    theme(aspect.ratio = 2, legend.position="none") +
    scale_x_continuous(limits=c(0,NA), expand=c(0,0)) +
    xlab("# significant genes") +
    ggtitle("Sex")
```

# One cell type
```{r one, fig.height=7, fig.width=12}
library(cowplot)
CT = names(fit)[1]

fig1 = plotDispEsts(fit, cluster_ids = CT )
fig2 = plotVolcano(fit, coef='age', cluster_ids = CT)
fig3 = plotVarPart(sortCols(vp), cluster_ids = CT)
fig4 = plotTrendVP( fit, vp, "CountNoise", cluster_ids = CT)

plot_grid(fig1, fig2, fig3, fig4, ncol=3)

fig5 = plotForest(fit, coef="age", gene = "ENSG00000197747" )
fig5 + theme_classic()  + theme(aspect.ratio=2) | 
figAge + theme_classic() + theme(aspect.ratio=2, legend.position="none")
```




<!---

# Other methods
```{r other, eval=FALSE}
source("/hpc/users/hoffmg01/work/lucida_analysis/simulations/de_methods.R")

methods = c(
    "nebula",
    "dreamlet",
    "DESeq2",
    "edgeR")

res.all = run_analysis(
      sce, 
      formula = ~ sex + age + (1|donor_id), 
      coefTest = "age",
      cluster_id = "cell_type", 
      methods = methods,
      nthreads = 4, 
      include_metadata = FALSE)
```


CT = "central memory CD4-positive, alpha-beta T cell"

countMatrix = as.matrix(counts(sceSub[,idx]))
id = data[[ran_var]]
ls = sceSub$libSize[idx]
save(countMatrix, id, ls, method, nthreads, design, file="~/test_nebula_small.RDATA")

#  devtools::install_github("GabrielHoffman/nebula", dep=FALSE)

q()
R
library(nebula)
load("~/test_nebula_small.RDATA")

keep = rowSums(countMatrix) > 0
keep = BatchRegression:::filter_responses(countMatrix, "nb", 0.001, 0.001)
sum(keep)


fit = nebula( 
      count = countMatrix[keep,], 
      id = id, 
      pred = design, 
      offset = ls, 
      method = method, 
      ncore = 1)

--->



#### Session info
<details>
```{r sessionInfo}
sessionInfo()
```
</details>
