...
```{r lucida.null}
fit.null <- lucida(sce, ~ (1|donor_id), cluster_id = "cell_type")
```
...
i = sce$cell_type %in% levels(sce$cell_type)[1:2]
j = sce$donor_id %in% levels(sce$donor_id)[1:30]
keep = i & j
...
design <- model.matrix(~ age, colData(pb))
