...
run_analysis <- function( sce.sim, formula, coefTest, cluster_id, methods, nthreads = 1, include_metadata = TRUE){
  ...
  if( "lucida" %in% methods ){
    ...
    fit.lucida <- lucida(sce.sim, formula, cluster_id, nthreads = nthreads, nReaders=min(nthreads, 8), verbose=FALSE)
    })
  ...
  if( "lucida [1 step]" %in% methods ){
    ...
    fit.lucida1 <- lucida(sce.sim, formula, cluster_id, shrinkDispersion=FALSE, nthreads = nthreads, nReaders=min(nthreads, 8), verbose=FALSE)
    })
