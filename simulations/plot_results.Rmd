...
df %>%
  filter(ID == 'ENSG00000142676', 
    cluster_id == "mucosal invariant T cell",
    Method == 'lucida') %>%
  head %>%
  data.frame
...
df %>%
  ...
  ggplot(aes(Method, n, fill=Method)) +
    ...
    facet_wrap(~libScaleFactor)

# keep only genes included in each result
target = length(unique(df$Method))
keep <- df %>%
  group_by(cluster_id, ID, simID, libScaleFactor) %>%
  summarize(n = n()) %>%
  mutate(ID2 = paste(cluster_id, ID, simID, libScaleFactor)) %>%
  filter( n == target ) %>%
  pull(ID2)

df <- df %>%
      filter(paste(cluster_id, ID, simID, libScaleFactor) %in% keep) %>%
...
ylim = .3
fig = df %>%
  # filter(libScaleFactor == "1", cluster_id == "mucosal invariant T cell") %>%
  droplevels %>%
  ...
  geom_segment(aes(x = .6, y = 0, xend = 1.5, yend = 0), color = "grey60", size = 5) +
  geom_segment(aes(x = 1.6, y = target, xend = 2.4, yend = target), color = "grey60", size = 5) +
  coord_cartesian(ylim=c(-ylim, ylim)) +
  facet_grid(cluster_id ~ libScaleFactor ) 
...
df %>%
  ...
  ggplot(aes(theta.y, pmin(1e4, theta.x), color=Method)) +
    ...
    scale_color_manual(values = colorMethods) +
    facet_grid(libScaleFactor ~ cluster_id + Method)
...
df %>%
  ...
  ggplot(aes(mu, (log(theta.x) - log(theta.y))^2, color=Method)) +
    ...
    facet_grid(cluster_id ~ libScaleFactor) +
    scale_y_continuous() +
    coord_cartesian(expand=FALSE, ylim=c(-2, 5)) 
...
```{r theta.normalized, cache=TRUE, fig.width=18, fig.height=12}
df %>%
  filter(!is.na(theta.x)) %>%
  filter(Method != "lucida [Bayesian]") %>%
...
```{r theta.normalized2, cache=TRUE, fig.width=13, fig.height=12}
df %>%
  filter(!is.na(theta.x)) %>%
  filter(Method != "lucida [Bayesian]") %>%
  ggplot(aes(mu, (log(mu^2/theta.y) - log(mu^2/theta.x))^2, color=Method)) +
    geom_smooth(se=FALSE) +
    thm +    
    scale_color_manual(values = colorMethods) +
    xlab("Mean normalized counts") +
...
## AUPR
```{r AUPR, cache=TRUE, fig.height=8, fig.width=8}
# AUPR
