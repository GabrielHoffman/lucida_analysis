...
df %>%
  group_by(Method, libScaleFactor, N) %>%
  summarize(n = n()) %>%
...
# scp minerva:/sc/arion/work/hoffmg01/lucida_analysis/simulations/df.paruqet .
...
df %>%
  ...
  ggplot(aes(mu, dev, color=Method)) +
    geom_smooth(se=TRUE, alpha = .15, aes(fill=Method)) +
    thm +    
    scale_color_manual(values = colorMethods) +
    scale_fill_manual(values = colorMethods) +
    xlab("Mean normalized counts") +
    ...
    scale_x_log10() +
    facet_grid(N ~ cluster_id) +
    coord_cartesian(expand=FALSE, ylim=c(-0.5, 5)) 
...
df %>% 
  ...
  ggplot(aes(libScaleFactor , rMSE, fill=Method)) +
    ...
    ylab("Root mean squared error (sqrt scale)") +
