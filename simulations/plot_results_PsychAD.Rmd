...
# Time
--->
```{r time, fig.height=5, fig.width=12, eval=FALSE, echo=FALSE}
files = list.files("/sc/arion/scratch/hoffmg01/sims/PsychAD/constant", "res_sim_.*.parquet", full.names=TRUE)
...
df_time = mclapply(files, function(file){
  ...
  a %>%
    filter(!Method %in% c("pseudobulk", "pb2"))
...
# Resource usage
```{r mem.usage, fig.height=3, fig.width=8}
library(stringr)
files = list.files("/sc/arion/scratch/hoffmg01/sims/PsychAD/constant/logs/", "*out", full.names=TRUE)
