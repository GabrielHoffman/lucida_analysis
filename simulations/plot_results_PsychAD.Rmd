...
# Resource usage
```{r mem.usage, fig.height=3.5, fig.width=8}
library(stringr)
...
```{r theta.normalized, fig.width=18, fig.height=12, eval=FALSE, eval=FALSE}
df %>%
  filter(libScaleFactor == "1") %>%
  filter(N == "50") %>%
  filter(!is.na(theta.x)) %>%
...
```{r theta.normalized2, fig.width=13, fig.height=12, eval=FALSE}
df %>%
  filter(libScaleFactor == "1") %>%
  # filter(N == "100") %>%
  filter(!is.na(theta.x)) %>%
...
df %>%
  ...
  ggplot(aes(mu, abs((sigSq_g.x) - (sigSq_g.y)), color=Method)) +
    ...
    scale_x_log10() +
    facet_grid(N ~ cluster_id) +    
    coord_cartesian(expand=FALSE, ylim=c(0, .05))
```
