...
path = "/sc/arion/scratch/hoffmg01/sims/PsychAD/constant/8_12_16_20_25_50_.3/"
setwd(path)
...
if( length(target) > 0){
  ylim = quantile(df$logFC, c(.02, .98))
  df %>%
...
df %>%
  ...
  ggplot(aes(mu, (log(theta.x) - log(theta.y))^2, color=Method, fill=Method)) +
    ...
    scale_fill_manual(values = colorMethods) +
    xlab("Mean normalized counts") +
    ylab("Squared deviation of log theta (log10 scale)") +
    scale_x_log10() +
    scale_y_log10() +    
    facet_grid( N ~ cluster_id, scales="free_y") 
```
...
df %>%
  ...
  ggplot(aes(mu, abs((sigSq_g.x) - (sigSq_g.y)), color=Method, fill=Method)) +
    ...
    facet_wrap(~cluster_id) +
    xlab("Mean normalized counts") +    
    ylab("Squared deviation of sigSq_g (log10 scale)") +
    scale_x_log10() +
    scale_y_log10() +
    facet_grid(N ~ cluster_id, scales="free_y") +    
    coord_cartesian(expand=FALSE, ylim=c(0, NA))
...
## Results stratified by count magnitude
...
df %>% 
  ...
  mutate(bin = cut(log10(mu), breaks=breaks) %>%
    ...
    as.character) %>%
  filter(!is.na(bin)) %>%
  group_by(N, Method, cluster_id, bin, libScaleFactor) %>%
  filter(!is.na(FDR)) %>%
...
df %>% 
  ...
  filter(!is.na(FDR)) %>%
  summarize(aupr(b.true, -log10(FDR)), n = n()) %>%
  ggplot(aes(bin, F1.max, fill=Method)) +
    geom_bar(stat="identity", position="dodge") +
    thm +
    coord_cartesian(expand=FALSE, ylim=c(0,1)) +
    geom_hline(yintercept = 0.1, linetype="dashed", color="grey30", linewidth=1) +
