...
# run sims
cat $OUTFOLDER/script_sim.sh | parallel -P 60

# check that files were written
cat $OUTFOLDER/script_sim.sh | awk '{print $NF}' | xargs ls > /dev/null
...
# check that files were written
cat $OUTFOLDER/script_de.sh | awk '{print $NF}' | xargs ls > /dev/null 2> err.log

cat err.log | tr "'" " " | awk '{print $4}' | parallel basename {} .parquet > jobs.prefix

grep -f jobs.prefix $OUTFOLDER/script_de.sh | parallel -P60
...
cat $OUTFOLDER/script_de.sh | grep -v "sim_25_\|sim_50_\|sim_100_\|sim_250_" | parallel -P60
